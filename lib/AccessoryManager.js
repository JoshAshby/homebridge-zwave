...
class AccessoryManager {
      ...
      .forEach(service => {
        switch (service) {
          ...
          case 'TemperatureSensor':
            ...
            break
          case 'Dimmable':
            this._configureDimmableLightbulbService()
            break
          case 'Color':
            this._configureColorLightbulbService()
            break
        }
      })
  ...
  /**
   * Configure the "Dimmable" service for the accessory
   */
  _configureDimmableLightbulbService () {
    ...
    const onCharacteristicValueSet = value => {
      ...
      const setValue = brightnessCharacteristic.value

      if (setValue === 1 || setValue === 100) return true
      return false
    }
    ...
    // Setup handler for when the value for the "Brightness" characteristic is requested by HomeKit
    const brightnessCharacteristicName = 'Brightness'
  ...
  /**
   * Configure the "Color" service for the accessory
   */
  _configureColorLightbulbService () {
    const lightBulbService = this._getService(this._hapService.Lightbulb)
    const onCharacteristic = lightBulbService.getCharacteristic(this._hapCharacteristic.On)
    ...
    const hueCharacteristicValueSet = value => {
      ...
      const rgb = HSVtoRGB(value, saturation, brightness)

      return RGBWtoHex(rgb)
    }
    ...
    const colorCharacteristicZwaveNodeValueCriteria = {
      commandClass: 51, // COMMAND_CLASS_ZIP_ADV_SERVER
      valueIndex: SWITCH_BINARY_INDEX_SWITCH
    }
    ...
    const saturationCharacteristicValueSet = value => {
      ...
      const rgb = HSVtoRGB(hue, value, brightness)
    ...
    this._updateCharacteristicValueOnZwaveNodeValueUpdated(
      hueCharacteristic,
      hueCharacteristicName,
      hueCharacteristicZwaveNodeValueCriteria,
      Number
    )
    ...
    this._updateCharacteristicValueOnZwaveNodeValueUpdated(
      saturationCharacteristic,
      saturationCharacteristicName,
      saturationCharacteristicZwaveNodeValueCriteria,
      Number
    )
